 ...
 * ══════════════════════════════════════════════════════
 * Splash, Toast, Modal, التنقل، تلوين النتائج، سجل النشاط
 *
...
// إزاي تستخدم ui.js
// ══════════════════════════════════════════════════════
// - showToast('رسالة', true/false)  → رسالة مؤقتة خضرا/حمرا
// - closeMov('cfMov')               → أغلق أي نافذة منبثقة
// - gotoSc('records')               → انتقل لشاشة معينة
// - autoBloodResult('bl-hb', 12, 17)→ auto-detect طبيعي/غير طبيعي
// - autoBpResult()                  → auto-detect ضغط الدم
// - autoRbsResult('sc')             → auto-detect السكر
// - sR(selectElement)               → لوّن الـ select حسب النتيجة
...
// ════════════════════════════════════════════════════════════════════
// 16. INIT — تهيئة الصفحة عند الفتح + مستمعي Firebase
//    📌 Firebase listeners: records / staff / config
//    ✅ أي تغيير في Firebase يظهر فوراً على الجهاز بدون refresh
//    ⚠️ لا تضيف هنا — فضّل الأقسام اللي فوق
// ════════════════════════════════════════════════════════════════════
...
if(dbRef){
  dbRef.child('records').on('value', snap => {
    let data = snap.val();
    if(data){
  ...
  dbRef.child(STORAGE.STAFF).on('value', snap => {
    let data = snap.val();
    if(data){
      staff = Object.values(data);
...
function setGlobalDay(d){ curDay=d; document.getElementById('db1').classList.toggle('active',d===1); document.getElementById('db2').classList.toggle('active',d===2); if(activeTabId){ tabs[activeTabId].day=d; saveTabData(activeTabId); } }
