...
function recToFormData(dept, rec) {
  ...
  d[px + '-name'] = rec.name || '';
  d[px + '-age'] = rec.age || '';
  d[px + '-gender'] = rec.gender || '';
  d[px + '-ticket'] = rec.ticket || '';
  d[px + '-phone'] = rec.phone || '';
  d[px + '-nid'] = rec.nid || '';
  ...
  if (dept === 'blood') {
    ['hp', 'rbc', 'hct', 'wbc', 'plt', 'hb', 'rbs', 'hba1c', 'blood', 'rh', 'uric', 'urea', 'creat',
      ...
      'rf', 'crp', 'hcv', 'hbsag', 'hav', 'bt', 'ct', 'esr'
    ].forEach(f => d['bl-' + f] = rec[f] || '');
    ['hp', 'rbc', 'hct', 'wbc', 'plt', 'hb', 'rbs', 'hba1c', 'uric', 'urea', 'creat', 'tp', 'alb',
      'bili', 'sgot', 'sgpt', 'ldl', 'hdl', 'tg', 'chol', 'bt', 'ct', 'esr'
    ].forEach(f => d['bl-' + f + '-r'] = rec[f + 'R'] || '');
  } else if (dept === 'para') {
    ['urates', 'phosphate', 'oxalates', 'hpylori', 'bilharzia', 'trichomonas', 'ameba',
      'giardia', 'hnana', 'oxyuris', 'ascaris'
    ].forEach(f => d['pa-' + f] = rec[f] || '');
    d['pa-preg'] = rec.preg || '';
    d['pa-sugar'] = rec.sugar || '';
    d['pa-alb'] = rec.palb || '';
    d['pa-bili'] = rec.pbili || '';
    d['pa-acetone'] = rec.acetone || '';
  } else if (dept === 'xray') {
    d['xr-code'] = rec.code || '';
    d['xr-ename'] = rec.examname || '';
    d['xr-result'] = rec.result || '';
    d['xr-remark'] = rec.remark || '';
  } else if (dept === 'screen') {
    d['sc-smoke'] = rec.smoke || '';
    d['sc-hx-sugar'] = rec.hxSugar || '';
    d['sc-hx-bp'] = rec.hxBp || '';
    d['sc-bp-sys'] = rec['bp-sys'] || rec.bpSys || '';
    d['sc-bp-dia'] = rec['bp-dia'] || rec.bpDia || '';
    d['sc-bp-r'] = rec.bpR || '';
    d['sc-rbs'] = rec.rbs || '';
    d['sc-rbs-r'] = rec.rbsR || '';
    d['sc-hba1c'] = rec.hba1c || '';
    d['sc-hba1c-r'] = rec.hba1cR || '';
    d['sc-action'] = rec.action || '';
    d['sc-refer'] = rec.refer || '';
  }
  return d;
}

function openNewTab(dept, editRec) {
...
function saveCurrentTab(dept) {
  ...
  // طلب رقم التليفون في حالة النتائج الغير طبيعية (قسم افحص واطمن)
  const screenAbnormal = dept === 'screen' && (
    ABNORMAL_VALS.has(r.bpR || '') ||
    ABNORMAL_VALS.has(r.rbsR || '') ||
    ABNORMAL_VALS.has(r.hba1cR || '')
  );
