...
function hasAbnormal(r) {
  ...
  for (let k of keys) {
    if (k.endsWith('R') || k === 'result' || k === 'bpR' || k === 'rbsR' || k === 'hba1cR') {
      if (ABNORMAL_VALS.has(String(r[k] || '').trim())) {
        return true;
      }
    }
  }
...
function testCell(val, status) {
  ...
  return `<td style="background:${bg};text-align:center;padding:5px 6px;border-bottom:1px solid #eee;min-width:50px">
...
function renderRec() {
  let w = document.getElementById('recWrap');
  let cnt = document.getElementById('recCnt');
  let srch = (document.getElementById('srch')?.value || '').trim().toLowerCase();
  ...
  allDepts.forEach(dept => {
    ...
    // SCREEN
      ...
      html += `<thead><tr>
        <th>#</th><th>الاسم</th><th>السن</th><th>الجنس</th><th>التذكرة</th><th>التليفون</th><th>التاريخ</th><th>اليوم</th>
        <th>تدخين</th><th>تاريخ سكر</th><th>تاريخ ضغط</th>
      ...
      items.forEach((r, i) => {
        ...
        html += `<tr style="background:${isAb?'#fff8f8':i%2?'#fafafa':'#fff'}">
          ...
          <td style="text-align:center;font-size:11px">${r.hxSugar||'—'}</td>
          <td style="text-align:center;font-size:11px">${r.hxBp||'—'}</td>
