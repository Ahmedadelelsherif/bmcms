 ...
 * ══════════════════════════════════════════════════════
 * جداول السجلات بكل تفاصيلها، صلاحيات التعديل والحذف
 *
...
// ══════════════════════════════════════════════════════
// إزاي تستخدم records.js
// ══════════════════════════════════════════════════════
// - renderRec()           → ارسم جداول السجلات (بيُستدعى تلقائياً)
// - setRecFilter('blood') → فلتر: all/blood/para/xray/screen/abnormal
// - editRec(id)           → افتح سجل للتعديل (مشرف أو مدير فقط)
// - delRec(id)            → احذف سجل (مدير فقط)
// - clearAllRecords()     → امسح كل البيانات (مدير فقط مع تأكيد)
// - hasAbnormal(r)        → هل السجل فيه نتيجة غير طبيعية؟
//
// تصميم الخلايا:
// - testCell(val, status) → خلية القيمة + الحالة في مكان واحد (أخضر/أحمر)
// - qualCell(val)         → خلية للنتائج النوعية (موجب/سالب/إلخ)
//
// الفلاتر الموجودة:
// ┌──────────────────────────────────────────┐
// │ all │ blood │ para │ xray │ screen │ ⚠️  │
// └──────────────────────────────────────────┘
// ══════════════════════════════════════════════════════
...
function canEdit(r){
  ...
  return false; // data_entry can't edit
}
function canDelete(r){
  if(!curUser) return false;
  return curUser.role==='admin'; // only admin can delete
}
