 ...
 * ══════════════════════════════════════════════════════
 * دخول وخروج، إدارة الموظفين، قفل القافلة، صلاحيات
 *
...
// إزاي تستخدم auth.js
// ══════════════════════════════════════════════════════
// - doLogin()          → التحقق وتسجيل الدخول
// - logout()           → تسجيل الخروج
// - applyUser()        → ضبط الشاشة حسب دور المستخدم
// - addStaff()         → إضافة موظف (يحفظ Firebase فوراً)
// - removeStaff(i)     → حذف موظف
// - lockCaravan()      → قفل القافلة (ممنوع على الموظفين)
// - unlockCaravan()    → فتح القافلة
//
// أدوار المستخدمين (curUser.staffRole):
// - 'admin':       كل الصلاحيات
