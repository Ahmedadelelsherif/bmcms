...
function saveSetup() {
  let governorate = document.getElementById('s-governorate').value.trim();
  let center = document.getElementById('s-center').value.trim();
  let village = document.getElementById('s-village').value.trim();
  let coordinator = document.getElementById('s-coordinator').value.trim();
  let d1 = document.getElementById('s-d1').value;
  let d2 = document.getElementById('s-d2').value;
  
  if (!governorate || !center || !village || !coordinator || !d1) {
    showToast('⚠️ املأ جميع الحقول المطلوبة', false);
    return;
  }
  
  session = { governorate, center, village, coordinator, day1: d1, day2: d2 };
  localStorage.setItem(STORAGE.SESSION, JSON.stringify(session));
  if (dbRef) fbWrite('config', { session });
  applySessUI();
  showToast('✅ تم حفظ بيانات القافلة');
}
...
function applyUser() {
  ...
  if (session) { applySessUI(); gotoSc('entry'); } else { gotoSc('setup'); }
  showToast('✅ مرحباً');
}
...
function applySessUI() {
  ...
  document.getElementById('sb-governorate').textContent = session.governorate || '—';
  document.getElementById('sb-center').textContent = session.center || '—';
  document.getElementById('sb-vil').textContent = session.village || '—';
  document.getElementById('sb-coordinator').textContent = session.coordinator || '—';
}
...
function updateLockUI() {
  ...
  if (locked) { l.style.display = 'none'; u.style.display = 'block'; b.innerHTML = '<div style="background:#fff0f0;padding:8px;color:#7a1a1a;text-align:center">🔒 مغلقة</div>'; } else { l.style.display = 'block'; u.style.display = 'none'; b.innerHTML = '<div style="background:#f0fff4;padding:8px;color:#1a5a2a;text-align:center">✅ مفتوحة</div>'; }
}
