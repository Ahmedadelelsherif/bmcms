...
function generateReportText() {
  ...
  let abn = records.filter(r => {
    if (r.dept === 'blood') return ABNORMAL_VALS.has(r.hbR) || ABNORMAL_VALS.has(r.rbsR);
    if (r.dept === 'para') return ABNORMAL_VALS.has(r.bilharzia) || ABNORMAL_VALS.has(r.ameba);
    if (r.dept === 'xray') return ABNORMAL_VALS.has(r.result);
    if (r.dept === 'screen') return ABNORMAL_VALS.has(r.bpR) || ABNORMAL_VALS.has(r.rbsR);
    return false;
  });
...
function exportPDF() {
  ...
  showToast('📄 جاري فتح معاينة الطباعة...', true);
...
function exportExcel() {
  ...
  let headerRows = [
    ['وزارة الصحة والسكان'],
    ['مديرية الشؤون الصحية بمحافظة ' + (session?.governorate || 'البحيرة')],
    ['إدارة القوافل العلاجية'],
    [''],
    ['المحافظة: ' + (session?.governorate || '—'), 'المركز: ' + (session?.center || '—'), 'القرية: ' + (session?.village || '—')],
    ['منسق القوافل: ' + (session?.coordinator || '—'), 'اليوم الأول: ' + (session?.day1 || '—'), 'اليوم الثاني: ' + (session?.day2 || '—')],
    [''],
    ['تقرير القافلة - ' + new Date().toLocaleDateString('ar-EG')],
    ['']
  ];

  let summary = headerRows.concat([
    [''],
    ['القسم', 'الإجمالي']
  ]);
  ...
  summary.push(['معمل الدم', b.length]);
  summary.push(['الطفيليات', p.length]);
  summary.push(['الأشعة', x.length]);
  summary.push(['افحص واطمن', s.length]);
...
function exportDeptExcel(dept) {
  ...
  const deptConfig = {
    blood: {
      hdr: [...baseHdr, 'العيادة', 'Hb', 'ن Hb', 'RBS', 'ن RBS', 'HbA1c', 'ن HbA1c', 'فصيلة', 'Rh', 'HP', 'ن HP', 'RBC', 'ن RBC', 'HCT', 'ن HCT', 'WBC', 'ن WBC', 'PLT', 'ن PLT', 'يوريك', 'ن', 'بولينا', 'ن', 'كرياتنين', 'ن', 'TP', 'ن', 'ألبيومين', 'ن', 'بيليروبين', 'ن', 'SGOT', 'ن', 'SGPT', 'ن', 'LDL', 'ن', 'HDL', 'ن', 'TG', 'ن', 'كوليسترول', 'ن', 'H.Pylori', 'حمل', 'ASOT', 'RF', 'CRP', 'HCV', 'HBsAg', 'HAV', 'BT', 'ن BT', 'CT', 'ن CT', 'ESR', 'ن ESR'],
      row: r => [r.clinic || '', r.hb || '', r.hbR || '', r.rbs || '', r.rbsR || '', r.hba1c || '', r.hba1cR || '', r.blood || '', r.rh || '', r.hp || '', r.hpR || '', r.rbc || '', r.rbcR || '', r.hct || '', r.hctR || '', r.wbc || '', r.wbcR || '', r.plt || '', r.pltR || '', r.uric || '', r.uricR || '', r.urea || '', r.ureaR || '', r.creat || '', r.creatR || '', r.tp || '', r.tpR || '', r.alb || '', r.albR || '', r.bili || '', r.biliR || '', r.sgot || '', r.sgotR || '', r.sgpt || '', r.sgptR || '', r.ldl || '', r.ldlR || '', r.hdl || '', r.hdlR || '', r.tg || '', r.tgR || '', r.chol || '', r.cholR || '', r.hpylori || '', r.preg || '', r.asot || '', r.rf || '', r.crp || '', r.hcv || '', r.hbsag || '', r.hav || '', r.bt || '', r.btR || '', r.ct || '', r.ctR || '', r.esr || '', r.esrR || '']
    },
    para: {
      hdr: [...baseHdr, 'Urates', 'Phosphate', 'Oxalates', 'حمل بول', 'سكر بول', 'ألبيومين بول', 'بيليروبين بول', 'أسيتون', 'H.Pylori', 'بلهارسيا', 'تريكومونس', 'أميبا', 'جيارديا', 'H.nana', 'أكسيورس', 'أسكارس'],
    ...
    xray: {
      hdr: [...baseHdr, 'العيادة', 'كود الفحص', 'اسم الفحص', 'النتيجة', 'ملاحظات'],
      row: r => [r.clinic || '', r.code || '', r.examname || '', r.result || '', r.remark || '']
    },
    screen: {
      hdr: [...baseHdr, 'تدخين', 'تاريخ سكر', 'تاريخ ضغط', 'انقباضي', 'انبساطي', 'نتيجة الضغط', 'سكر RBS', 'نتيجة السكر', 'HbA1c', 'نتيجة HbA1c', 'الإجراء', 'تحويل'],
